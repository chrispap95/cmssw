...
public:
  FWSimClusterProxyBuilder(void) {}
  ~FWSimClusterProxyBuilder(void) override {}
...
private:
  ...
  FWSimClusterProxyBuilder(const FWSimClusterProxyBuilder &) = delete;                   // stop default
  const FWSimClusterProxyBuilder &operator=(const FWSimClusterProxyBuilder &) = delete;  // stop default

  void setItem(const FWEventItem *iItem) override;

  void build(const FWEventItem *iItem, TEveElementList *product, const FWViewContext *vc) override;
  void build(const SimCluster &iData,
             unsigned int iIndex,
             TEveElement &oItemHolder,
             const FWViewContext *) override;
};

void FWSimClusterProxyBuilder::setItem(const FWEventItem *iItem) {
  FWHeatmapProxyBuilderTemplate::setItem(iItem);
  if (iItem) {
    iItem->getConfig()->assertParam("Cluster(0)/SimHit(1)", false);
...
void FWSimClusterProxyBuilder::build(const FWEventItem *iItem, TEveElementList *product, const FWViewContext *vc) {
...
void FWSimClusterProxyBuilder::build(const SimCluster &iData,
                                      unsigned int iIndex,
                                      TEveElement &oItemHolder,
                                      const FWViewContext *) {
  ...
  std::vector<std::pair<uint32_t, float>> clusterDetIds = iData.hits_and_fractions();
  ...
  for (std::vector<std::pair<uint32_t, float>>::iterator it = clusterDetIds.begin(), itEnd = clusterDetIds.end();
       it != itEnd;
       ++it) {
...
REGISTER_FWPROXYBUILDER(FWSimClusterProxyBuilder, SimCluster, "Sim Cluster", FWViewType::kISpyBit);
